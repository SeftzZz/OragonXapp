...
@Component({
  ...
  templateUrl: "cron-bid.page.html",
  styleUrls: ["cron-bid.page.scss"],
})
export class CronBidPage {
  ...
  async ngOnInit() {
    ...
    this.senddata.getcurrentbnb().subscribe(
      (data: any) => {
    ...
    if (this.localStorage == undefined) {
      ...
      this.senddata.balancePlayers(this.wallets).subscribe(
        (dataB: any) => {
          var Web3 = require("web3");
          this.balanceORGNPlayers = (
            Web3.utils.fromWei(dataB.result, "gwei") * 1
          )
            .toFixed(2)
            .replace(/\d(?=(\d{3})+\.)/g, "$&,");
      ...
      this.senddata.balanceBNBPlayers(this.wallets).subscribe(
        (dataB: any) => {
          var Web3 = require("web3");
          this.balanceBNBPlayers = (
            Web3.utils.fromWei(dataB.result, "gwei") * 1
          )
            .toFixed(2)
            .replace(/\d(?=(\d{3})+\.)/g, "$&,");
      ...
      this.senddata.balanceBUSDPlayers(this.wallets).subscribe(
        (dataB: any) => {
          var Web3 = require("web3");
          this.balanceBUSDPlayers = (
            Web3.utils.fromWei(dataB.result, "gwei") * 1
          )
            .toFixed(2)
            .replace(/\d(?=(\d{3})+\.)/g, "$&,");
      ...
      for(let i in this.eggsBID) {
        if(this.isRunning == false) {
          this.countdown = setInterval(() => {
            ...
            this.marketp2pBIDeggDocId = this.eggsBID[i].DocId;
            this.hoursEgg = this.eggsBID[i].hours;
            this.minutesEgg = this.eggsBID[i].minutes;
            this.secondsEgg = this.eggsBID[i].seconds;
            this.secondsEgg++;

            if (this.secondsEgg >= 60) {
              this.minutesEgg++;
              this.secondsEgg = 0;
            }

            if (this.minutesEgg >= 60) {
              this.hoursEgg++;
              this.minutesEgg = 0;
            }

            this.senddata.hmsBIDmp(this.marketp2pBIDeggDocId, this.hoursEgg, this.minutesEgg, this.secondsEgg).subscribe((res:any) => {
              this.isRunning = true;
              this.bidTime = true;
            });

            if(this.hoursEgg >= this.eggsBID[i].bidTime) {
              ...
              this.countdownSecond = setInterval(() => {
                this.senddata.getselleggBIDmp().subscribe((dataSell: any) => {
                  this.eggsBID = dataSell;
                });
              }, 1000);
  ...
  async pay_firstMarketp2p() {
    ...
    if (kind == "dragon") {
      ...
      // Transaction Data Firestore
      var sfDocRef = this.fs.firestore
        .collection("Sell")
        .doc(this.marketp2pfoodowner);
      ...
      try {
        this.fs.firestore.runTransaction(async (transaction: any) => {
          ...
          if (
            sfDoc
              .data()
              ["FoodGroup"].indexOf(JSON.stringify(this.current_data)) > -1
          ) {
            // console.log(sfDoc)
            ...
            await transaction.update(sfDocRef, {
              FoodGroup: firebase.firestore.FieldValue.arrayRemove(
            ...
            await transaction.update(sfDocRef2, {
              Food: firebase.firestore.FieldValue.arrayUnion(
                JSON.stringify(datax)
              ),
              FoodLength: firebase.firestore.FieldValue.increment(
  ...
  async pay_cartBID() {
    ...
    if (kind == "dragon") {
      ...
      // this.senddata.setitemhistory(this.marketp2pfoodID,this.globalID,'transactionHash',JSON.stringify(this.current_data)).subscribe((data:any) => {},(error:any) => {});
      this.senddata.addSellListmp(this.marketp2pfoodID, this.globalID, this.input_price).subscribe((dataE: any) => {});
      this.senddata
        .updatesellfoodUserqtymp(
          this.globalID,
          this.marketp2pfoodID,
  ...
  async getfoodbuydetail(iditemssellFood) {
    let lastseen = localStorage.setItem("lastseen", "food");
    this.checkkindGroup(iditemssellFood);
  ...
  // DRAGON
    ...
    detailBid(ItemId) {
      this.senddata.getselldgidBIDmp(ItemId).subscribe(async (dataDetail: any) => {
        ...
        console.log("latest_date", this.latest_date);
        this.sesion1 = 0;
  ...
  // EGG
    ...
    detailBidegg(ItemId) {
      this.senddata.getselleggidBIDmp(ItemId).subscribe(async (dataDetail: any) => {
        ...
        this.marketp2pBIDeggImg = this.eggDetail[0].Id;
        this.marketp2pBIDeggAttack = this.eggDetail[0].Attack;
        this.marketp2pBIDeggDefense = this.eggDetail[0].Defense;
        this.marketp2pBIDeggLevel = this.eggDetail[0].Level;
        this.marketp2pBIDeggHP = this.eggDetail[0].HP;
        this.marketp2pBIDeggRarity = this.eggDetail[0].Rarity;
        this.marketp2pBIDeggExp = this.eggDetail[0].Exp;
        ...
        this.marketp2pBIDeggLastUpdate = this.eggDetail[0].EditAt;
        this.marketp2pBIDeggbidTime = this.eggDetail[0].bidTime;
        this.marketp2peggBIDfavorited = this.eggDetail[0].favorited;
        this.marketp2pBIDeggBG = this.eggDetail[0].imgbackground;
        this.marketp2pBIDeggImgBG = this.eggDetail[0].imgBg;
        let marketp2peggpriceUSD = this.marketp2pBIDeggpriceNow / this.current_bnb;
        this.marketp2peggpriceUSD = marketp2peggpriceUSD.toFixed(2);
        console.log(this.marketp2pBIDeggpriceNow);
        console.log("type", this.marketp2pBIDeggtype);
        ...
        if(this.isRunning == false) {
          this.countdown = setInterval(async () => {
            this.senddata.gethmsBIDmp(this.marketp2pBIDeggDocId).subscribe((res:any) => {
              let hms = JSON.parse(res);
              this.hoursEgg = hms.hours;
              this.minutesEgg = hms.minutes;
              this.secondsEgg = hms.seconds;
              console.log(this.hoursEgg, this.minutesEgg, this.secondsEgg);
              this.secondsEgg++;
              if (this.secondsEgg >= 60) {
                this.minutesEgg++;
                this.secondsEgg = 0;
              ...
              this.senddata.hmsBIDmp(this.marketp2pBIDeggDocId, this.hoursEgg.toString(), this.minutesEgg.toString(), this.secondsEgg.toString()).subscribe((res:any) => {
                this.isRunning = true;
                // this.bidTime = true;
              });

              if(this.hoursEgg >= this.marketp2pBIDeggbidTime) {
                // this.bidTime = false;
  ...
  // BATTERY
    ...
    detailBidbattery(ItemId) {
      this.senddata.getsellbatteryidBIDmp(ItemId).subscribe(async (dataDetail: any) => {
        ...
        this.marketp2pBIDbatteryImg = this.batteryDetail[0].Id;
        this.marketp2pBIDbatteryAttack = this.batteryDetail[0].Attack;
        this.marketp2pBIDbatteryDefense = this.batteryDetail[0].Defense;
        this.marketp2pBIDbatteryLevel = this.batteryDetail[0].Level;
        this.marketp2pBIDbatteryHP = this.batteryDetail[0].HP;
        this.marketp2pBIDbatteryRarity = this.batteryDetail[0].Rarity;
        this.marketp2pBIDbatteryExp = this.batteryDetail[0].Exp;
        ...
        this.marketp2pbatteryBIDfavorited = this.batteryDetail[0].favorited;
        this.marketp2pBIDbatteryBG = this.batteryDetail[0].imgbackground;
        this.marketp2pBIDbatteryImgBG = this.batteryDetail[0].imgBg;
        ...
        if(this.latest_date > this.marketp2pBIDbatterytime) {
    ...
    async countDownBIDBattery() {
      ...
      // If the count down is over, write some text
      if (distance < 0 || this.hoursBattery < 0) {
        this.hoursBattery = 0;
        this.minutesBattery = 0;
        this.secondsBattery = 0;
        this.marketp2pbatterytimeCount = 0;
        // console.log(this.marketp2pdragontimeCount);
        // this.senddata.getselldgBIDmp().subscribe(
        //   async (dataSell: any) => {
        //     this.dragonsBID = dataSell;
  ...
  checkkindGroup(iditemssellFood) {
    let lastseen = localStorage.getItem("lastseen");
    this.senddata.getsellfoodidmp(iditemssellFood).subscribe(
      (dataDetail: any) => {
